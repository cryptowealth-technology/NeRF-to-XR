...
def rays_from_camera(use_pixel_centers, h, w, focal, camtoworlds):
    ...
    camera_dirs = np.stack(
        [(x - w * 0.5) / focal, -(y - h * 0.5) / focal, -np.ones_like(x)], axis=-1
    )
    directions = (
        camera_dirs[None, Ellipsis, None, :] * camtoworlds[:, None, None, :3, :3]
    ).sum(axis=-1)
...
class Dataset(threading.Thread):
    ...
    def _generate_rays(self):
        ...
        origins, directions, viewdirs = rays_from_camera(
            self.use_pixel_centers, self.h, self.w, self.focal, self.camtoworlds
        )
