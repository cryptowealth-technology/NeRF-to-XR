...
def synchronize_jax_hosts():
    ...
    gathered_array = np.reshape(gathered_array[0], (num_hosts, num_local_devices, 1))
...
def export_snerg_scene(
    ...
    for i in range(0, atlas.shape[2], 4):
        ...
        for j in range(4):
            plane_index = i + j
            rgb_stack.append(
                atlas[:, :, plane_index, :][Ellipsis, 0:3].transpose([1, 0, 2])
            )
            alpha_stack.append(
                atlas[:, :, plane_index, :][
                    Ellipsis, scene_params["_channels"]
                ].transpose([1, 0])
            )
    ...
    atlas_index_image = (
        np.transpose(atlas_block_indices, [2, 1, 0, 3])
        .reshape((-1, atlas_block_indices.shape[0], 3))
        .astype(np.uint8)
    )
    ...
    for i, rgb_and_alpha in enumerate(zip(rgbs, alphas)):
        ...
        rgba = np.minimum(
            uint_multiplier, np.maximum(0.0, np.floor(uint_multiplier * rgba))
        ).astype(np.uint8)
    ...
    for i in range(0, atlas.shape[2], 4):
        ...
        for j in range(4):
            plane_index = i + j
            feature_slice = atlas[:, :, plane_index, :][Ellipsis, 3:-1].transpose(
                [1, 0, 2]
            )
            feature_slice = np.minimum(
                uint_multiplier,
                np.maximum(0.0, np.floor(uint_multiplier * feature_slice)),
            ).astype(np.uint8)
...
def compute_scene_size(
    output_directory, atlas_block_indices, atlas_params, scene_params
):
    ...
    block_index_size_gb = np.array(atlas_block_indices.shape).prod() / (
        1000 * 1000 * 1000
    )

    active_atlas_blocks = (atlas_block_indices[Ellipsis, 0] >= 0).sum()
    active_atlas_voxels = active_atlas_blocks * atlas_params["atlas_block_size"] ** 3
    active_atlas_channels = active_atlas_voxels * scene_params["_channels"]

    byte_size_gb = active_atlas_channels / (1000 * 1000 * 1000) + block_index_size_gb
    float_size_gb = (
        active_atlas_channels * 4 / (1000 * 1000 * 1000) + block_index_size_gb
    )
