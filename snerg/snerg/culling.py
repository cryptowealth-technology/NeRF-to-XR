...
def crop_alpha_grid(
    ...
    if not (
        np.max(active_voxels_x) and np.max(active_voxels_y) and np.max(active_voxels_y)
    ):
    ...
    min_in_voxels = (min_in_voxels - avg_in_voxels) * dilation_factor + avg_in_voxels
    max_in_voxels = (max_in_voxels - avg_in_voxels) * dilation_factor + avg_in_voxels

    min_in_world = (
        min_in_voxels * culling_params["_voxel_size"] + scene_params["min_xyz"]
    )
    max_in_world = (1.0 + max_in_voxels) * culling_params["_voxel_size"] + scene_params[
        "min_xyz"
    ]
    ...
    params.post_process_params(
        render_params_cropped,
        culling_params_cropped,
        atlas_params_cropped,
...
def integrate_visibility_from_rays(
    origins, directions, alpha_grid, visibility_grid, scene_params, grid_params
):
    ...
    for i in range(num_steps):
        ...
        for dx, dy, dz in offsets_xyz:
            ...
            visibility_grid.ravel()[
                np.ravel_multi_index(grid_indices, visibility_grid.shape)
...
def integrate_visibility_from_image(
    h, w, focal, camtoworld, alpha_grid, visibility_grid, scene_params, grid_params
):
...
def max_downsample_grid(culling_params, atlas_params, culling_grid_values):
    ...
    atlas_grid_maximum = np.zeros(
        (
            atlas_params["_atlas_grid_size"][0],
            atlas_params["_atlas_grid_size"][1],
    ...
    for block_x in range(atlas_params["_atlas_grid_size"][0]):
        for block_y in range(atlas_params["_atlas_grid_size"][1]):
            for block_z in range(atlas_params["_atlas_grid_size"][2]):
                min_voxel = (
                    np.array([block_x, block_y, block_z]) * culling_params["block_size"]
                )
                max_voxel = np.minimum(
                    min_voxel + culling_params["block_size"],
                ...
                block_values = culling_grid_values[
