...
class ImageQualityEvaluator:
    ...
    def __init__(self):
        ...
        self._ssim_fn = jax.jit(
            functools.partial(utils.compute_ssim, max_val=1.0), backend="cpu"
        )
    ...
    def psnr(self, image1, image2):
        return -10.0 * np.log(np.mean((image1 - image2) ** 2)) / np.log(10.0)

    def eval_image_list(self, images1, images2):
        """Returns the average PSNR, SSIM and LPIPS between each pair of images."""
        psnrs = [self.psnr(image1, image2) for image1, image2 in zip(images1, images2)]
        ssims = [self.ssim(image1, image2) for image1, image2 in zip(images1, images2)]
...
def build_sharded_dataset_for_view_dependence(
    source_dataset,
    atlas_t,
    ...
    for i in range(num_batches):
        ...
        viewdirs = np.zeros((source_dataset.h, source_dataset.w, 3), dtype=np.float32)

        if dataset_index < num_images:
            rgb, alpha = rendering.atlas_raymarch_image_tf(
                source_dataset.h,
                source_dataset.w,
                source_dataset.focal,
                source_dataset.camtoworlds[dataset_index],
                atlas_t,
                atlas_block_indices_t,
                atlas_params,
                scene_params,
            ...
            _, _, viewdirs = datasets.rays_from_camera(
                scene_params["_use_pixel_centers"],
                source_dataset.h,
                source_dataset.w,
                source_dataset.focal,
        ...
        if scene_params["white_bkgd"]:
            np_rgb[Ellipsis, 0:3] = (
                np.ones_like(np_rgb[Ellipsis, 0:3]) * (1.0 - np_alpha)
                + np_rgb[Ellipsis, 0:3]
            )
...
def refine_view_dependence_mlp(
    rgbs,
    directions,
    refs,
    viewdir_mlp_model,
    ...
    def train_step(model, state, ref, rgb_features, directions, lr):
        ...
        def loss_fn(variables):
            residual = model_utils.viewdir_fn(
                model, variables, rgb_features, directions, scene_params
            )
    ...
    batch_aware_learning_rate = (
        learning_rate * jax.device_count()
    ) / num_local_devices_used_for_tuning
    for _ in range(num_epochs):
        for i in range(rgbs.shape[0]):
            state, _ = train_pstep(
                state, refs[i], rgbs[i], directions[i], batch_aware_learning_rate
            )
