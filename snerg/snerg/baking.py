...
def build_3d_grid(
    ...
    x_span = (
        min_xyz[0]
        + voxel_size / 2
        + np.arange(grid_size[0], dtype=output_dtype) * voxel_size
    )
    y_span = (
        min_xyz[1]
        + voxel_size / 2
        + np.arange(grid_size[1], dtype=output_dtype) * voxel_size
    )
    z_span = (
        min_xyz[2]
        + voxel_size / 2
        + np.arange(grid_size[2], dtype=output_dtype) * voxel_size
    )
...
def render_voxel_block(
    mlp_model, mlp_params, block_coordinates_world, voxel_size, scene_params
):
    ...
    for i in range(origins.shape[0]):
        ...
        host_batch_origins = batch_origins[
            host_id
            * jax.local_device_count() : (host_id + 1)
            * jax.local_device_count()
        ]

        batch_rgb, batch_sigma = model_utils.pmap_model_fn(
            mlp_model, mlp_params, host_batch_origins, scene_params
        )
        rgb_and_features[i] = np.array(batch_rgb[0], dtype=output_dtype).reshape(
            rgb_and_features[i].shape
        )
    ...
    output_rgb_and_features = rgb_and_features.reshape(
        (
            block_coordinates_world.shape[0],
            block_coordinates_world.shape[1],
            block_coordinates_world.shape[2],
            channels,
        )
    )
...
def extract_3d_atlas(
    ...
    atlas_blocks = np.logical_and(
        ...
        alpha_grid_values >= culling_params["alpha_threshold"],
    ).sum()
    atlas_slices = (atlas_blocks * atlas_block_size ** 3) / (atlas_slice_size ** 2)
    atlas_slices = atlas_block_size * int(np.ceil(atlas_slices / atlas_block_size))
    ...
    for block_x in range(atlas_grid_size[0]):
        for block_y in range(atlas_grid_size[1]):
            for block_z in range(atlas_grid_size[2]):
                ...
                if (
                    alpha_grid_values[block_x, block_y, block_z]
                    < culling_params["alpha_threshold"]
                ):
                    continue

                if (
                    visibility_grid_values[block_x, block_y, block_z]
                    < culling_params["visibility_threshold"]
                ):
                    continue

                atlas_min_voxel = (
                    np.array([block_x, block_y, block_z]) * data_block_size - 1
                )
                ...
                atlas_block_indices[block_x, block_y, block_z] = np.array(
                    [atlas_ix, atlas_iy, atlas_iz]
                )
                atlas_world_coordinates[
                    atlas_x : (atlas_x + atlas_block_size),
    ...
    for _ in range(num_samples):
        ...
        for z in range(atlas.shape[2]):
            atlas_rgb, atlas_alpha = render_voxel_block(
                mlp_model,
                mlp_params,
                atlas_world_coordinates[:, :, z : z + 1, :] + offset,
                voxel_size,
                scene_params,
            )
