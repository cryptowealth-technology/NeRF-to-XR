...
def main(unused_argv):
    ...
    (
        ...
        atlas_params_init,
        scene_params_init,
    ) = params.initialize_params(FLAGS)
    ...
    while True:
        ...
        (
            ...
            viewdir_mlp_model,
            viewdir_mlp_params,
        ) = model_utils.extract_snerg_mlps(state.optimizer.target, scene_params_init)

        # Render out the low-res grid used for culling.
        culling_grid_coordinates = baking.build_3d_grid(
            scene_params_init["min_xyz"],
            culling_params_init["_voxel_size"],
        ...
        culling_grid_coordinates = baking.build_3d_grid(
            scene_params["min_xyz"],
            culling_params["_voxel_size"],
        ...
        for camera_index in range(
            0, num_training_cameras, culling_params["visibility_subsample_factor"]
        ):
            culling.integrate_visibility_from_image(
                ...
                train_dataset.camtoworlds[camera_index],
                culling_grid_alpha,
                culling_grid_visibility,
                scene_params,
                culling_params,
            )
        ...
        atlas_grid_alpha = culling.max_downsample_grid(
            culling_params, atlas_params, culling_grid_alpha
        )
        ...
        atlas_grid_visibility = ndimage.maximum_filter(
            ...
            culling_params["visibility_grid_dilation"],
        ).astype(atlas_grid_visibility.dtype)
        ...
        for i in range(test_dataset.camtoworlds.shape[0]):
            renders_and_paths.append(
                (
                    post_refined_images[i],
                    path.join(out_render_dir, "{:03d}.png".format(i)),
                )
            )
