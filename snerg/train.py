...
def train_step(model, rng, state, batch, lr):
    ...
    def loss_fn(variables):
        ...
        if len(ret) > 1:
            ...
            sparsity_c = (
                FLAGS.sparsity_strength * jax.numpy.log(1.0 + sigma_c ** 2 / 0.5).mean()
            )
        else:
            loss_c = 0.0
            psnr_c = 0.0
        ...
        sparsity = (
            FLAGS.sparsity_strength * jax.numpy.log(1.0 + sigma ** 2 / 0.5).mean()
        )
        stats = utils.Stats(
            loss=loss,
            psnr=psnr,
        ...
        if jax.host_id() == 0:
            if step % FLAGS.print_every == 0:
                ...
                summary_writer.scalar(
                    "train_sparsity_coarse", stats.sparsity_c[0], step
                )
