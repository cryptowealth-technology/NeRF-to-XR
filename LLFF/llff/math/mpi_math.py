...
def myposes2mats(poses, fix_yx=False):
    with tf.variable_scope("myposes2mats"):
        ...
        with tf.variable_scope("Kstuff"):
            K = stk([f, m_z, -w * 0.5, m_z, f, -h * 0.5, m_z, m_z, -m_o], -1)
            K = tf.reshape(K, sh)
        ...
        T_c2w, T_w2c, K, K_inv = map(
            lambda x: tf.cast(x, dtype=poses.dtype), [T_c2w, T_w2c, K, K_inv]
        )
...
def plane_homogs(pose_t, pose_s, depths, planes_from_t=True, y_flip=True, fix_yx=False):
    with tf.variable_scope("plane_homogs"):
        ...
        n = tf.constant(np.array([0, 0, 1.0]), dtype=T_t2s.dtype)
        n = tf.reshape(n, [3, 1])
        nT = tf.transpose(n)
        ...
        if y_flip:
            premat = tf.stack([1, 0, 0.0, 0, -1, pose_t[0, -1] - 1, 0, 0, 1.0], -1)
            premat = tf.cast(premat, dtype=T_t2s.dtype)
            premat = tf.reshape(premat, [3, 3])

            postmat = tf.stack([1, 0, 0.0, 0, -1, pose_s[0, -1] - 1, 0, 0, 1.0], -1)
...
homog_render_fn = lambda data, homog, window=None: homog_warp(
    data, homog, window=window
)


def render_mpi_homogs(
    mpi_rgba, pose, newpose, min_disp, max_disp, num_depths, debug=False
):
    with tf.variable_scope("render_mpi_homogs"):
        ...
        H = plane_homogs(
            newpose, pose, 1.0 / dispvals, planes_from_t=False, y_flip=True, fix_yx=True
        )
        H = tf.reshape(H, [-1, 9])
        H = H[:, :8] / H[:, 8:]
        ...
        weights = mpiR_alpha * tf.cumprod(
            1.0 - mpiR_alpha + ALPHA_EPS, -2, exclusive=True, reverse=True
        )  # goddamnit pratul
...
def make_psv_homogs(img, pose, newpose, dispvals, num_depths, window=None):
    with tf.variable_scope("make_cv_homogs"):
        ...
        H = plane_homogs(
            newpose, pose, 1.0 / dispvals, planes_from_t=True, y_flip=True, fix_yx=True
        )  # [N, D, 3, 3]
        ...
        cvd = tf.squeeze(
            tf.transpose(cvd, [2, 3, 1, 4, 0])
        )  # [H, W, D, 3, N] or [H, W, D, 3]
...
class Pose:
    def __init__(self, m, window=None):  # , h, w, focal, mat):
        ...
        t = self.mat[:3, 3]
        RT = tf.transpose(R)
        self.invmat = tf.concat(
            [RT, -tf.reduce_sum(RT * t, axis=-1, keep_dims=True)], axis=-1
        )
        ...
        if window is None:
            ...
            h0 = tf.cond(window[3] > 0, lambda: window[0], lambda: 0.0)
            w0 = tf.cond(window[3] > 0, lambda: window[1], lambda: 0.0)
        ...
        self.xf = (self.w - 0) / self.f * 0.5
        self.yf = (self.h - 0) / self.f * 0.5
        yvals = (tf.to_float(tf.range(h0, h0 + h1)) - self.h * 0.5) / self.f
        xvals = (tf.to_float(tf.range(w0, w0 + w1)) - self.w * 0.5) / self.f
    ...
    def project_in(self, pts, clip=False):
        ...
        pts_y = (pts[..., 0] / self.yf + 1.0) * self.h * 0.5
        pts_x = (pts[..., 1] / self.xf + 1.0) * self.w * 0.5
        if clip:
            pts_y = tf.clip_by_value(pts_y, 0.0, self.h - 1.0)
            pts_x = tf.clip_by_value(pts_x, 0.0, self.w - 1.0)
...
def warp(img, pose, newpose, depth, single=False, debug=False):
    ...
    mask_y = tf.logical_and(pix_y <= pose.h - 1.0, pix_y >= 0.0)
    mask_x = tf.logical_and(pix_x <= pose.w - 1.0, pix_x >= 0.0)
    mask = tf.to_float(tf.logical_and(mask_x, mask_y))

    pix_y = tf.clip_by_value(pix_y, 0.0, pose.h - 1.0)
    pix_x = tf.clip_by_value(pix_x, 0.0, pose.w - 1.0)
