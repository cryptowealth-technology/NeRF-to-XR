...
def generate_render_path(poses, bds, comps=None, N=30):
    ...
    close_depth, inf_depth = bds[0, :].min() * 0.9, bds[1, :].max() * 5.0
    dt = 0.75
    ...
    rads[2] = zdelta
    if comps[3]:
        render_poses += render_path_spiral(c2w, up, rads, focal, zdelta, 0.0, 1, N * 2)
    if comps[4]:
        render_poses += render_path_spiral(c2w, up, rads, focal, zdelta, 0.5, 2, N * 4)
...
def render_path_fig(poses, render_poses, scaling_factor=1.0, savepath=None):
    c2w = poses_avg(poses)
