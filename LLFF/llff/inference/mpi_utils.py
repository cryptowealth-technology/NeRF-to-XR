...
class MPI:
    ...
    def save(self, basedir, txt_only=False, binary=False):
        ...
        with open(os.path.join(basedir, "metadata.txt"), "w") as file:
            file.write(
                "{} {} {} {}\n".format(
                    mpi.shape[0], mpi.shape[1], mpi.shape[2], pose[2, -1]
                )
            )
            for j in range(4):
                file.write("{} {} {}\n".format(*[pose[i, j] for i in range(3)]))
...
def load_mpi(basedir):
    ...
    data = np.frombuffer(open(mpibinary, "rb").read(), dtype=np.uint8) / 255.0
    data = data.reshape([d, h, w, 4]).transpose([1, 2, 0, 3])
    data[..., -1] = np.minimum(1.0, data[..., -1] + 1e-8)
...
def run_inference(
    imgs, poses, mpi_bds, ibr_runner, num_planes, patched=False, disps=False, psvs=False
):
    keys = ["mpi0", "disps", "psv"]

    generator = lambda inputs: ibr_runner.run_inference(
        inputs, test_keys=keys, patched=patched, valid=270, buffer=80
    )
    mpis = []
...
def render_mpis(
    ...
    for i, p in enumerate(render_poses):
        ...
        if alpha_blend:
            outframe = (rends[..., :3] * weights).sum(0) / (
                1e-10 + (rends[..., -1:] * weights).sum(0)
            )
        else:
