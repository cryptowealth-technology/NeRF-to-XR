...
parser.add_argument(
    "--checkpoint",
    type=str,
    default="./checkpoints/papermodel/checkpoint",
    help="pretrained network checkpoint",
)
parser.add_argument(
    "--factor", type=int, help="integer downsampling factor for input images"
)
parser.add_argument("--width", type=int, help="output mpi width (pixels)")
parser.add_argument("--height", type=int, help="output mpi height (pixels)")
parser.add_argument("--numplanes", type=int, default=32, help="output mpi depth")
...
def gen_mpis(basedir, savedir, fwh, logdir, num_planes, no_mpis, disps, psvs):
    ...
    N = imgs.shape[-1]
    close_depths = [bds.min() * 0.9] * N
    inf_depths = [bds.max() * 2.0] * N
    mpi_bds = np.array([close_depths, inf_depths])

    mpis = run_inference(
        imgs, poses, mpi_bds, ibr_runner, num_planes, patched, disps=disps, psvs=psvs
    )
    ...
    for i in range(N):
        ...
        if disps:
            plt.imsave(
                os.path.join(savedir, "mpi{:02d}/disps.png".format(i)), mpis[i].disps
            )
        if psvs:
            psv = np.moveaxis(mpis[i].psv, -2, 0)
            psv = (255 * np.clip(psv, 0, 1)).astype(np.uint8)
            imageio.mimwrite(
                os.path.join(savedir, "mpi{:02d}/psv.mp4".format(i)),
                psv,
                fps=30,
                quality=5,
            )
