...
parser.add_argument("mpidir", type=str, help="input mpi directory")
parser.add_argument("posefile", type=str, help="input render poses file")
parser.add_argument("videofile", type=str, help="output video file")
parser.add_argument(
    "--use_N", type=int, default=5, help="number of mpis to blend per output frame"
)
parser.add_argument(
    "--crop_factor",
    type=float,
...
def render_video(mpidir, renderpathfile, outputfile, use_N):
    ...
    print(
        "Rendering {} poses, crop factor {}, res {}, mpi shape {}".format(
            render_poses.shape[0],
            args.crop_factor,
            render_poses[0, :2, -1].astype(np.int32),
            mpis[0].mpi.shape,
        )
    )

    outframes = render_mpis(
        mpis, render_poses, use_N=use_N, weight_scale=1.0, alpha_blend=True
    )
